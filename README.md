...
> Muốn đổi luật (VD dùng cho khu vực Hải Phòng): sửa duy nhất object `ADVANCE` ở đầu file `public/rules.js` — mọi màn hình tự cập nhật theo.
...
2. Kéo cả **thư mục `public/`** vào khung upload (hoặc dùng `npx vercel deploy public --prod`)
...
```
Rồi ở Vercel: **Add New → Project → Import repo**, để **mặc định toàn bộ** (Framework Preset: *Other*, Build Command: *để trống*, Output Directory: `public`) → Deploy.

**Cách 3 – CLI:**
```bash
npm i -g vercel
vercel --prod        # Output Directory: public
```

Chạy thử ở máy: `python3 -m http.server 8080 -d public` → mở http://localhost:8080
...
```
Script ghi lại `public/data.js`. Nếu lịch chưa liệt kê hết các cặp vòng tròn, script **tự sinh thêm** các trận còn thiếu (để trống giờ/sân, web hiện dấu `—`).
...
public/
  ...
  app.js        UI 4 tab, nhập điểm, export
